...
from qat_utils import QuantizedModel,model_equivalence
...
parser.add_argument('--degree', default="int8", type=str)
...
def main(args):
    ...
    save_folder_quantized = f"results_quantization/{args.dataset_name}_{args.model_name}_{args.sparsity}"

    print(f"Save Folder: {save_folder_quantized}")
    if not os.path.exists(save_folder_quantized):
        os.makedirs(save_folder_quantized)
    ...
    quantized_victim_model = BaseModel(args.dataset_name, args.model_name, warm=args.warm, iter_per_epoch=iter_per_epoch, num_cls=args.num_cls, input_dim=args.input_dim, lr=quantized_lr,
        weight_decay=args.weight_decay, save_folder=save_folder_quantized_victim, device=device, optimizer=args.optimizer, epochs = args.quantized_epochs)
    ...
    assert model_equivalence(
            model_1=quantized_victim_model.model,
            model_2=fused_victim_model.model,
            device=device,
            rtol=1e-03,
            atol=1e-06,
            num_tests=100,
    ...
    quantized_victim_model.model = QuantizedModel(model_fp32=fused_victim_model.model)
    ...
    for epoch in range(args.quantized_epochs):
        ...
        if test_acc > best_acc:
            ...
            if count > args.early_stop:
                ...
                break
    victim_quantized_acc = best_acc
    shutil.copyfile(best_path, f"{save_folder_quantized_victim}/best.pth")



    # quantized shadow models
    shadow_acc_list = []
    shadow_quantized_acc_list = []
    for shadow_ind in range(args.shadow_num):
        ...
        shadow_quantized_model = BaseModel(args.dataset_name, args.model_name, warm=args.warm, iter_per_epoch=iter_per_epoch, 
                                            num_cls=args.num_cls, input_dim=args.input_dim, lr=quantized_lr, weight_decay=args.weight_decay,
                                            save_folder=quantized_shadow_model_save_folder, device=device, optimizer=args.optimizer, epochs = args.quantized_epochs)
        ...
        assert model_equivalence(
            model_1=shadow_quantized_model.model,
            model_2=fused_shadow_model.model,
            device=device,
            rtol=1e-03,
            atol=1e-06,
            num_tests=100,
        ...
        shadow_quantized_model.model = QuantizedModel(model_fp32=fused_shadow_model.model)
        ...
        for epoch in range(args.quantized_epochs):
            ...
            if test_acc > best_acc:
                ...
                if count > args.early_stop:
                    print(f"Early Stop at Epoch {epoch}")
                    break
        shutil.copyfile(best_path, f"{quantized_shadow_model_save_folder}/best.pth")
