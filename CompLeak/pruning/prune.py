...
from prune_utils import get_pruner
...
parser.add_argument('--prune_epochs', default=30, type=int)
...
def main(args):
    ...
    save_folder_pruned = f"results_pruning/{args.dataset_name}_{args.model_name}_{args.prune_sparsity}"

    print(f"Save Folder: {save_folder_pruned}")
    if not os.path.exists(save_folder_pruned):
        os.makedirs(save_folder_pruned)
    ...
    victim_pruned_model = BaseModel(
        args.dataset_name, args.model_name, warm=args.warm, iter_per_epoch=iter_per_epoch, num_cls=args.num_cls, input_dim=args.input_dim, save_folder=save_folder_pruned_victim,
        device=device, optimizer=args.optimizer, lr=prune_lr, weight_decay=args.weight_decay, epochs = args.prune_epochs)
    ...
    for shadow_ind in range(args.shadow_num):
        ...
        pruned_shadow_model_save_folder = f"{save_folder_pruned}/shadow_model_{shadow_ind}"
        ...
        if args.warm:
            ...
            iter_per_epoch = 0
        shadow_pruned_model = BaseModel(args.dataset_name, args.model_name, warm=args.warm, iter_per_epoch=iter_per_epoch, 
                                 num_cls=args.num_cls, input_dim=args.input_dim, save_folder=pruned_shadow_model_save_folder, device=device, optimizer=args.optimizer, lr=prune_lr, weight_decay=args.weight_decay, epochs = args.prune_epochs)
        ...
        for epoch in range(args.prune_epochs):
            pruner.update_epoch(epoch)
            train_acc, train_loss = shadow_pruned_model.train(epoch, attack_train_loader, f"Shadow {shadow_ind} Epoch {epoch} Prune Train")
            test_acc, test_loss = shadow_pruned_model.test(attack_test_loader, f"Shadow {shadow_ind} Epoch {epoch} Prune Test")
            if test_acc > best_acc:
                best_acc = test_acc
