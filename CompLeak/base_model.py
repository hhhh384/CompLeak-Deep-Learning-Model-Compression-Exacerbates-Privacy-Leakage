...
import copy
from utils import get_model, WarmUpLR, get_optimizer
...
class BaseModel:
    def __init__(self, dataset_name, model_type, warm = 0, iter_per_epoch=0, device="cuda", save_folder="", num_cls=10,
                optimizer="sgd", lr=0.1, weight_decay=5e-4, input_dim=100, epochs=100):
    ...
    def cluster_train(self, epoch, trainloader):
        ...
        print("Train: Accuracy {:.3f}, Loss {:.3f}".format(acc, total_loss))
        return acc, total_loss




    def test_SAMIA(self, test_loader, log_pref=""):
        self.model.eval()
        total_loss = 0
        correct = 0
        total = 0
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(test_loader):
                ...
                all_pred_probs.append(outputs.softmax(dim=1).cpu().numpy())
                all_ground_truth.append(targets.cpu().numpy())

        all_pred_probs = np.concatenate(all_pred_probs, axis=0)
        all_ground_truth = np.concatenate(all_ground_truth, axis=0)

        if all_pred_probs.shape[1] == 2:  
            auc1 = roc_auc_score(all_ground_truth, all_pred_probs[:, 1])  
        else: 
            auc1 = roc_auc_score(all_ground_truth, all_pred_probs, multi_class='ovr', average='macro')  

        fpr, tpr, thresholds = roc_curve(all_ground_truth, all_pred_probs[:, 1]) 
        fpr_target = 0.001
        ...
        if log_pref:
            print("{}: Accuracy {:.3f}, Loss {:.3f}, AUC {:.4f}, TPR@0.1% FPR {:.4f}".format(log_pref, acc, total_loss, auc1, tpr_at_low_fpr))

        return acc, total_loss, auc1, tpr_at_low_fpr
    ...
    def loss_target_predict(self, data_loader):
